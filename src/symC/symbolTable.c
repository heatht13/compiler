...
private:
    ...
    std::map<const char* , void *> symbols;    // use an ordered map (not as fast as unordered)

public:
    Scope(const char *newname);
    ~Scope();
    const char * scopeName();                   // returns name of scope
    void debug(int state);                    // sets the debug flag to state
    void print(void (*printData)(void *));     // prints the table using the supplied function to print the void *
    void applyToAll(void (*action)(const char *, void *));  // applies func to all symbol/data pairs
    int insert(const char * sym, void *ptr);   // inserts a new ptr associated with symbol sym
...
SymbolTable::Scope::Scope(const char *newname) {
    name = newname;
    debugFlg = false;
}
...
// set scope debugging
void SymbolTable::Scope::debug(int state) {
    debugFlg = state;
}
...
void SymbolTable::Scope::print(void (*printData)(void *)) {
    printf("Scope: %-15s -----------------\n", name.c_str());
    for (std::map<const char * , void *>::iterator it=symbols.begin(); it!=symbols.end(); it++) {
...
void SymbolTable::Scope::applyToAll(void (*action)(const char *, void *)) {
    for (std::map<const char * , void *>::iterator it=symbols.begin(); it!=symbols.end(); it++) {
...
bool SymbolTable::Scope::insert(const char *sym, void *ptr) {
...
void *SymbolTable::Scope::lookup(const char * sym) {
    if (symbols.find(sym) != symbols.end()) {
...
int  SymbolTable::Scope::debugFlg;
...
{
    debugFlg = false;
    enter((const char * )"Global");
}


void SymbolTable::debug(int state)
{
    debugFlg = state;
}
...
void * SymbolTable::lookup(const char *sym)
{
    void *data;
    const char * name;
    ...
    if (debugFlg) {
        printf("DEBUG(SymbolTable): lookup the symbol \"%s\" and ", sym);
...
// returns NULL if symbol not found, otherwise it returns the stored void * associated with the symbol
void * SymbolTable::lookupGlobal(const char * sym)
{
    void *data;
...
int SymbolTable::insert(const char * sym, void *ptr)
{
    if (debugFlg) {
...
int SymbolTable::insertGlobal(const char * sym, void *ptr)
{
    if (debugFlg) {
...
void SymbolTable::applyToAll(void (*action)(const char *, void *))
{
...
// string and the associated pointer.
void SymbolTable::applyToAllGlobal(void (*action)(const char *, void *))
{
